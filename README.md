...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/Th3Ya0vi/openclaw-1a)
...
For E2E testing that needs to test the real device pairing flow:

```bash
E2E_TEST_MODE=true          # Skip Cloudflare Access auth but keep device pairing enabled
```
...
For local development only, set `DEV_MODE=true` in `.dev.vars` to skip Cloudflare Access authentication and enable `allowInsecureAuth` (bypasses device pairing entirely). For E2E tests that need to test the real pairing flow, use `E2E_TEST_MODE=true` instead (skips CF Access auth but keeps device pairing enabled).
...
You can route API requests through [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/) for caching, rate limiting, analytics, and cost tracking. AI Gateway supports multiple providers — configure your preferred provider in the gateway.
...
2. Add Anthropic as a provider to your gateway
...
You can also use [OpenRouter](https://openrouter.ai/) via AI Gateway to access alternative models like GLM 4.7 (their free model is `openrouter/auto`):

1. Create an AI Gateway and add OpenRouter as a Universal Endpoint
2. In your AI Gateway settings, enable **API Shield** and generate an authorization token
3. Set the gateway secrets:
...
# Your AI Gateway endpoint URL (note the /openrouter suffix)
npx wrangler secret put AI_GATEWAY_BASE_URL
# Enter: https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/openrouter

# AI Gateway authorization token (from API Shield)
...
When using OpenRouter, the gateway will automatically configure GLM 4.7 models.
...
| `CF_AIG_AUTHORIZATION`  | No       | AI Gateway authorization token from API Shield (required for OpenRouter)                                   |
...
| `E2E_TEST_MODE`         | No       | Set to `true` to skip CF Access auth but keep device pairing (for automated E2E tests)                     |
