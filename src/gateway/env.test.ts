...
describe("buildEnvVars", () => {
  ...
  it("passes AI_GATEWAY_API_KEY and CF_AIG_AUTHORIZATION for OpenRouter config", () => {
    const env = createMockEnv({
      AI_GATEWAY_API_KEY: "sk-or-v1-openrouter-key",
      AI_GATEWAY_BASE_URL:
        "https://gateway.ai.cloudflare.com/v1/123/my-gw/openrouter",
      CF_AIG_AUTHORIZATION: "cf-auth-token",
    });
    const result = buildEnvVars(env);
    // Should pass both the mapped key and original key for start-moltbot.sh OpenRouter detection
    expect(result.AI_GATEWAY_API_KEY).toBe("sk-or-v1-openrouter-key");
    expect(result.AI_GATEWAY_BASE_URL).toBe(
      "https://gateway.ai.cloudflare.com/v1/123/my-gw/openrouter"
    );
    expect(result.CF_AIG_AUTHORIZATION).toBe("cf-auth-token");
    // Also maps to ANTHROPIC since it's not /openai endpoint
    expect(result.ANTHROPIC_API_KEY).toBe("sk-or-v1-openrouter-key");
    expect(result.ANTHROPIC_BASE_URL).toBe(
      "https://gateway.ai.cloudflare.com/v1/123/my-gw/openrouter"
    );
  });
