...
const isOpenRouter = baseUrl.includes('openrouter') || baseUrl.includes('completions');
const aiGatewayApiKey = process.env.AI_GATEWAY_API_KEY || '';
const cfAigAuthorization = process.env.CF_AIG_AUTHORIZATION || '';
...
if (isOpenAI) {
    ...
    // Clean up providers that shouldn't exist when using OpenAI
    delete config.models.providers.anthropic;
...
} else if (isOpenRouter && aiGatewayApiKey && cfAigAuthorization) {
    console.log('Configuring OpenRouter provider with base URL:', baseUrl);
    config.models = config.models || {};
    config.models.providers = config.models.providers || {};
    // Clean up providers that shouldn't exist when using OpenRouter
    delete config.models.providers.anthropic;
    delete config.models.providers.openai;
    // OpenRouter is OpenAI Chat Completions compatible; use openai-responses (not anthropic-messages)
    config.models.providers.openrouter = {
        ...
        auth: 'api-key',
        headers: {
            'cf-aig-authorization': 'Bearer ' + cfAigAuthorization
        },
        authHeader: true,
        models: [
            { id: 'openrouter/auto', name: 'OR Auto', contextWindow: 200000 },
        ]
    };
    // Add models to the allowlist so they appear in /models
    config.agents.defaults.models = config.agents.defaults.models || {};
    config.agents.defaults.models['openrouter/auto'] = { alias: 'OR Auto' };
    config.agents.defaults.model.primary = 'openrouter/auto';
} else if (baseUrl && !isOpenRouter) {
    console.log('Configuring Anthropic provider with base URL:', baseUrl);
    config.models = config.models || {};
    config.models.providers = config.models.providers || {};
    // Clean up providers that shouldn't exist when using Anthropic
    delete config.models.providers.openai;
    delete config.models.providers.openrouter;
    ...
    // Clean up any custom providers from previous runs
